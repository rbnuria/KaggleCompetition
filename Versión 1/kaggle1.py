...
##### VERSIÓN 1:
## Quitamos solo las variables que tengan más de la mitad de valores perdidos -> No hemos estudiado todavía la correlación de las variables

#Sustituimos Nans en Functional por la moda
#En el resto de Bsmt que no estaban ya sustituidos hacemos lo mismo que para los que si lo estaban: NA means No basement
#No garage Area -> 0.0 garage area
# Garage cond igual que el resto de Garage "algo"
#GarageYrBlt -> año en el que fue fabricado por la media de los que si tienen valor
# Adding needed libraries and reading data
# Si no tiene MasVnARea -> sustituimos por 0
#Utilities -> por la moda
...
Final_labels_train = (np.exp(GB_model.predict(train_features)) + np.exp(ENST_model.predict(train_features_st))) / 2


get_score(Final_labels_train, np.exp(train_labels))

## Saving to CSV
pd.DataFrame({'Id': test.Id, 'SalePrice': Final_labels}).to_csv('submission1.csv', index =False)