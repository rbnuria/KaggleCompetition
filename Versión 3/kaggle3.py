...
##### VERSIÓN 3:
##  Añadimos sobre la versión 1: Cambiamos gradient-boostign por XGBoosting, aunque hemos obtenido que gradient boosting obtiene mejores resultados
#Diferencias GBM vs XGBOOST
...
xgbTest = xgb.XGBRegressor(n_estimators=3000, learning_rate=0.05)
...
## Getting our SalePrice estimatio
Final_labels = (np.exp(ENST_model.predict(test_features_st)) + np.exp(XGB_model.predict(test_features))) / 2
Final_labels_train = (np.exp(ENST_model.predict(train_features_st)) + np.exp(XGB_model.predict(train_features))) / 2
