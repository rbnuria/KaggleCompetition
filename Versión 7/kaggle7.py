...
Final_labels_train = (np.exp(ENST_model.predict(train_features)) + np.exp(GB_model.predict(train_features))) / 2

get_score(Final_labels_train, np.exp(train_labels))


## Saving to CSV
#pd.DataFrame({'Id': test.Id, 'SalePrice': Final_labels}).to_csv('submission5.csv', index =False)
