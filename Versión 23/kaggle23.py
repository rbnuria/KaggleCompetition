...
'''
    LASSO
'''
lasso = make_pipeline(RobustScaler(), Lasso(alpha =0.0005, random_state=1)).fit(x_train_st, y_train_st)
...
KRR = KernelRidge(alpha=0.2, kernel='polynomial', degree=2, coef0=1).fit(x_train_st, y_train_st)
...
Final_labels = (np.exp(GB_model.predict(test_features)) + np.exp(ENST_model.predict(test_features_st)) + np.exp(lasso_model.predict(test_features_st)) + np.exp(KRR_model.predict(test_features_st)) ) / 4
...
## Saving to CSV
#pd.DataFrame({'Id': test.Id, 'SalePrice': Final_labels}).to_csv('submission22.csv', index =False)